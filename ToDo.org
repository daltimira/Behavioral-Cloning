...
6- Implement pre-processing of the image data (see the notes.txt)
- Image resize by 2.
- Convert to gray scale.
DONE Crop.
DONE Normalize.

7- Implement image augmentation (see notes.txt). Flip horizontally and reverse steering angles.

8- Add left and right images with corrected steering angles.

DONE - Use generators to process data on the fly (not store in memory). See notes.txt.

9- Implement the model arquitecture of NVIDIA.

10- Add regularization (dropouts).

11- Save a model, and be able to reload the model to add new more data for further refinement ( see LabTransferLearning.txt).
